...
* `tagged-key-thumbprint-type`: a `digest` (e.g., the SHA-2 hash) of a raw public key.
  The digest value can be used to locate a public key contained in a lookup table, for example. Ultimately, the discovered keys SHOULD be byte-comparable with the corresponding keys. As an alternative, the digests can be byte-compared.
